...
export default class AnalysisLaboratory extends Component {
            ...
            axios.post(Config.ServerUri + "/get_graph_data", { data, token: token })
                .then((res) => {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        ...
                        x_axis.push({
                            id: res.data[i].material._id,
                            date: res.data[i].charge[res.data[i].charge.length - 1].date
                        })
                    }
                    ...
                    if (input_id.length > 0) {
                        axios.post(process.env.REACT_APP_API_URL + "analysis/input_history", { labId: input_id })
                            .then(async (history) => {
                                ...
                                var dataset_list = [];
                                // console.log(">>>>>>>>>>>>", graph_show_list)
                                graph_show_list.map((item, index) => {
                                    var randomColor = Math.floor(Math.random() * 16777215).toString(16);
                                    var option = this.state.selected_combinations[index];
                                    var option_values = this.state.selected_combinations[index].value.split('-');
                                    dataset_list.push(
                                        {
                                            material_id: option_values[option_values.length - 1],
                                            label: option.label.replace('-', ' / ') + ' (' +
                                                this.state.materialsData.filter(m => m._id === option_values[option_values.length - 1])[0].material + ')',
                                            fill: false,
    ...
    render() {
        // const state = {
        //     labels: this.state.data_charge_date.map(d => d.date),
        //     datasets: this.state.dataset
        // }
        this.state.dataset.map((dataset, index) => {
            const data = {
                labels: this.state.data_charge_date.filter(d => String(d.id) === String(this.state.dataset[index].material_id)).map(dd => dd.date),
                datasets: [this.state.dataset[index]]
            }
        })
        var materialOption = [];
        ...
        this.state.combination_list.map((item) => (
            options.push({
                ...
                value: item.analysisId + '-' + item.objectiveId + '-' + item.unitId + '-' + item.material_id
            })
        ));
        var clientOption = [];
                ...
                <div className="row">
                    ...
                    <div className="col-sm-7">
                        <div className="row">
                            {
                                this.state.dataset.map((dataset, index) => (
                                    <div className={`${this.state.dataset.length === 1 ? 'col-12' : 'col-6'}`} key={index}>
                                        <CCard>
