...
export default class AdminMaterial extends Component {
  ...
  getObjectiveValue(id/*objective id*/, unit/*unit id*/, client, analysis) {
    ...
    if (analysis == undefined) {
      var retVal = { id: id, unit: unit, client: client, analysis: "", min: 0, max:0 };
    }
    else {
      var retVal = { id: id, unit: unit, client: client, analysis: analysis, min: 0, max: 0 };
    }

    for (var i = 0; i < values.length; i++) {

      if (values[i].id === id && values[i].unit === unit && values[i].client === client && values[i].analysis === analysis) {
        retVal = values[i];
        break;
      }
    }
  ...
  handleMultiSelectChange_A_Types(e, obj, client) {
    ...
    e.map((item) => {
      var ids = obj.split('-');
      objectiveValues.push(this.getObjectiveValue(ids[0], ids[1], client, item.value));
      analysisTypes.push({label: item.label, value: item.value, obj: obj, client: client, min: 0, max: 0});
      return true;
    })

    this.setState({ analysisTypes: analysisTypes, objectiveValues: objectiveValues });
  }
  ...
  getAnalysisName(id) {
    ...
    for (var i = 0; i < analysis.length; i++) {
      if (analysis[i]._id === id)
        return analysis[i].analysisType;
    }
    return '';
  }
  ...
  renderModalCreate() {
    ...
    this.state.filteredObjectives = Array.from(objectives.reduce((a, o) => a.set(`${o.value}-${o.client}`, o), new Map()).values());
      ...
      objs.map(temp => {
        item.objectives.map(item0 => {
          if (temp.value == item0.id + '-' + item0.unit) {
            analysisOptions.push({ label: item.analysisType, value: item._id, client: client.value, objective: item0.id + '-' + item0.unit});
          }
        })
      })
        ...
        <CCol md="10">
          ...
          </CFormGroup>
          {
            this.state.filteredObjectives.map((obj, objIdx) => {
              ...
              label = obj.label;
              if(label != '') {
                return this.renderATypes(client, obj, objIdx, _analysisOptions, _analyisTypes)
              }
              return true
            })            
          }
    ...
    return <CFormGroup key={objIdx}>
      ...
      </CRow>
      {
        this.state.analysisTypes.map((type, typeIdx) => {
          ...
          return this.renderminMaxValues(client, obj, type, typeIdx, label, objValue);
        })
      }
    </CFormGroup>
  }
    ...
    return (<CFormGroup key={typeIdx}>
      <CRow style={{marginTop: '13px'}}>
        ...
        <CCol md="1">
          ...
          <input type="number" name="min" style={{ width: "50px", height: "30px", borderColor: "lightseagreen", borderRadius: "5px" }} value={objValue.min} onChange={(e) => this.handleObjectiveInputChange(e, client, obj, type, typeIdx)} />
        </CCol>
        <CCol md="2">
          Max Value
        </CCol>
        <CCol md="1">
          ...
          <input type="number" name="max" style={{ width: "50px", height: "30px", borderColor: "lightseagreen", borderRadius: "5px" }} value={objValue.max} onChange={(e) => this.handleObjectiveInputChange(e, client, obj, type, typeIdx)} />
        </CCol>
      </CRow>      
  ...
  getTooltip(item, client) {
    ...
    item.objectiveValues.map((item0) => {
      ...
      console.log(item0);
      if (name !== '' && unit !== '') {
        ret = ret + analysis + '-' + name + ' ' + unit + ': ' + '[' +  item0.min + '-' + item0.max + ']' + ", ";
        count ++;
      }
  ...
  on_update_clicked(item) {
    ...
    item.objectiveValues.map(temp => {
      ...
      if (label !== '' && unit !== '' && clientIndex >= 0) {
        ...
        objectives.push({ label: label + " " + unit, value: temp.id + "-" + temp.unit, client: temp.client, analysis: temp.analysis});

        analysisTypes.push({ label: analysis, value: temp.analysis, obj: temp.id + '-' + temp.unit, client: temp.client, min: temp.min, max: temp.max });
  ...
  createMaterial(event) {
    ...
    objectiveValues.map(item => {
      analysisTypes.map(data => {
        if (item.client === data.client && item.analysis === data.value && item.id === data.obj.split('-')[0] && item.unit === data.obj.split('-')[1]) {
          item.min = data.min;
          item.max = data.max;
        }
      })
